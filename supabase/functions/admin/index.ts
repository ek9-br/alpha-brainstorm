...
import {fallbackGroups,groupingJsonSchema,GroupingIdea,validateSuggestions} from './grouping.ts';
...
async function aiGroups(ideas:GroupingIdea[]){
  ...
  if(!key)return {groups:fallbackGroups(ideas),provider:'fallback',reason:'OPENAI_API_KEY não configurada'};
  try{
    const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',signal:AbortSignal.timeout(45_000),headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model:Deno.env.get('OPENAI_GROUPING_MODEL')||'gpt-5.6-luna',reasoning:{effort:'low'},input:[{role:'system',content:'Agrupe contribuições semanticamente semelhantes. Nunca misture setores. Use cada ID exatamente uma vez. Gere títulos curtos e descrições objetivas em português. Retorne somente o JSON do schema.'},{role:'user',content:JSON.stringify({ideas})}],text:{format:{type:'json_schema',name:'brainstorm_groups',strict:true,schema:groupingJsonSchema}}})});
    if(!response.ok)throw new Error(`OpenAI HTTP ${response.status}`);
    const result=await response.json();
    const outputText=result.output?.flatMap((item:{content?:unknown[]})=>item.content||[]).find((item:{type?:string})=>item.type==='output_text')?.text;
    if(typeof outputText!=='string')throw new Error('Resposta sem output_text');
    return {groups:validateSuggestions(JSON.parse(outputText),ideas),provider:'openai'};
  }catch(error){return {groups:fallbackGroups(ideas),provider:'fallback',reason:error instanceof Error?error.message:'Falha desconhecida'}}
}
...
Deno.serve(async req=>{
  ...
  try{
    ...
    }else if(action==='suggest_groups'){
      if(session.status!=='AI_GROUPING')return json({error:'As sugestões só podem ser geradas durante o agrupamento.'},409);
      const [ideasResult,sourcesResult,areasResult]=await Promise.all([
        db.from('brainstorm_ideas').select('id,area_id,text').eq('session_id',session.id).order('created_at'),
        db.from('brainstorm_consolidated_idea_sources').select('idea_id'),
        db.from('brainstorm_areas').select('id,name').eq('active',true)
      ]);
      if(ideasResult.error||sourcesResult.error||areasResult.error)throw ideasResult.error||sourcesResult.error||areasResult.error;
      const linked=new Set((sourcesResult.data||[]).map(source=>source.idea_id));const areaNames=new Map((areasResult.data||[]).map(area=>[area.id,area.name]));
      const ideas:GroupingIdea[]=(ideasResult.data||[]).filter(idea=>!linked.has(idea.id)).map(idea=>({id:idea.id,area_id:idea.area_id,area_name:areaNames.get(idea.area_id)||'Setor',text:idea.text}));
      if(!ideas.length)return json({created:0,provider:'none',message:'Não há contribuições pendentes.'});
      const suggestion=await aiGroups(ideas);const created=[];const failures=[];
      for(const group of suggestion.groups){
        const {data,error}=await db.rpc('brainstorm_admin_create_group',{p_session_id:session.id,p_area_id:group.area_id,p_title:group.title,p_description:group.description,p_idea_ids:group.idea_ids});
        if(error){failures.push({title:group.title,error:error.message});continue}
        await db.from('brainstorm_consolidated_ideas').update({grouping_method:suggestion.provider==='openai'?'ai':'individual',grouping_confidence:suggestion.provider==='openai'?0.7:null}).eq('id',data.id).eq('session_id',session.id);
        created.push(data.id);
      }
      await db.from('brainstorm_admin_audit_log').insert({session_id:session.id,action,payload:{provider:suggestion.provider,created:created.length,failures:failures.length}});
      return json({created:created.length,provider:suggestion.provider,failures,reason:suggestion.reason});
