import {KeyboardEvent,PointerEvent,useMemo,useRef,useState,WheelEvent} from 'react';
import {ChevronRight,LocateFixed,Maximize2,Network,Search,ZoomIn,ZoomOut} from 'lucide-react';
...
export function AdminMindMap({workspace}:{workspace:AdminWorkspace}){
 ...
 const startPan=(event:PointerEvent<SVGSVGElement>)=>{if(event.target!==event.currentTarget)return;event.currentTarget.setPointerCapture(event.pointerId);panStart.current={clientX:event.clientX,clientY:event.clientY,viewX:view.x,viewY:view.y}};
 ...
 return <section className="mt-8">
  ...
  <div className="card mt-5 p-3 sm:p-5">
   ...
   <p className="mt-3 text-xs text-slate-500">Arraste o fundo para navegar, use a roda do mouse ou os botões para ampliar e clique em uma ideia para ver suas respostas originais.</p>
   <div className="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
    <svg className="h-[34rem] w-full cursor-grab touch-none active:cursor-grabbing sm:h-[38rem]" viewBox={`${view.x} ${view.y} ${view.width} ${view.height}`} role="img" aria-label="Mapa mental das ideias aprovadas para votação" onPointerDown={startPan} onPointerMove={movePan} onPointerUp={endPan} onPointerCancel={endPan} onWheel={handleWheel}>
