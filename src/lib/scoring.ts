...
export function score(votes:WeightedVote[]){const averages=Object.fromEntries(CRITERIA.map(([k])=>[k,weightedAverage(votes,k)])) as Ratings;const total=CRITERIA.reduce((sum,[key])=>sum+averages[key],0);return{averages,total,percentage:total/(CRITERIA.length*5)*100}}
